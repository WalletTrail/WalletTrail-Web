  ...
  return (
    <div className="security-page">
      ...
      <main>
        ...
        <section className="security-path" aria-label="Security guidance pathway"><div className="security-path-intro"><p className="eyebrow">Before you act</p><h2>Stop. Check what is being asked of you.</h2></div><div className="security-grid" aria-label="Security guidance">{securityTopics.map((topic, index) => <article className="security-card" key={topic.title}><div className="security-card-number">{String(index + 1).padStart(2, '0')}</div><p className="card-eyebrow">Security basics</p><h2>{topic.title}</h2><p>{topic.text}</p><strong>{topic.action}</strong></article>)}</div></section>
