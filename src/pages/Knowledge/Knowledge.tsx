...
export default function Knowledge() {
  ...
  return (
    <main className="knowledge-page">
      ...
      <section className="knowledge-map" aria-label="Knowledge areas">
        <div className="knowledge-map-core"><span>WalletTrail</span><strong>Knowledge</strong></div>
        <div className="knowledge-map-grid">
          {areas.map(([title, text, href], index) => <a className={`knowledge-map-node knowledge-map-node-${index + 1}`} href={href} key={title}><span>{String(index + 1).padStart(2, '0')}</span><strong>{title}</strong><small>{text}</small><em>Explore →</em></a>)}
        </div>
      </section>
